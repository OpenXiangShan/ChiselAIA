...
//latch awxx and arxx
///can use AXI4Bundle----
      val awaddr_l = RegEnable(awchvld,in.aw.bits.addr)
      val wdata_l = RegEnable(awchvld,in.w.bits.data)
      val awsize_l   = RegEnable(awchvld,in.aw.bits.size)
      val awburst_l   = RegEnable(awchvld,in.aw.bits.burst)
      val awlen_l   = RegEnable(awchvld,in.aw.bits.len)
      val awid_l   = RegEnable(awchvld,in.aw.bits.id)
      ...
      val sIDLE :: sWCH :: sBCH :: sRCH =Enum(4)
      val state = RegInit(sIDLE)
      val awchvld = isAWValid & in.w.valid
      val isFinalBurst = (state == sWCH) && (awcnt == awlen_l) && (wcnt == awlen_l) // the final data for a transaction
      switch(state){
        is(sIDLE) {
          when(awchvld){
            state := sWCH
      ...
      when(state === sWCH){
        when(awcnt >= awburst_l) {
          awcnt := awcnt
        }.elsewhen((!isillegalAW) & (awcnt ===0.U)){
          when(out.aw.ready){
            awcnt := awcnt + 1.U
          }.otherwise{
            awcnt := awcnt
          ...
          awcnt := awcnt + 1.U
        } // arrive the max length of burst
      }.elsewhen(state === sRCH){
        when(in.ar.ready){
          awcnt := awcnt + 1.U
        }
