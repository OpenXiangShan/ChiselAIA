...
class IMSICToCSRBundle(params: IMSICParams) extends Bundle {
  ...
  // add port: interrupt pending of ree when cmode is tee,else interrupt pending of tee.
  val notice_pending = Option.when(params.HasTEEIMSIC)(Output(Bool()))
}
  ...
  locally {
    ...
    when(fromCSR.addr.valid && !illegal_priv) // read
    {
      ...
      when(pv === Cat(PrivType.M.asUInt, false.B)) {
        intFilesSelOH_r := UIntToOH(0.U)
      }
        .elsewhen(pv === Cat(PrivType.S.asUInt, false.B)) {
          intFilesSelOH_r := UIntToOH(1.U)
        }
        .elsewhen(pv === Cat(PrivType.S.asUInt, true.B)) {
          intFilesSelOH_r := UIntToOH(1.U((fromCSR.vgein.getWidth + 1).W)
    ...
    when(fromCSR.addr.valid && fromCSR.wdata.valid && !(fromCSR.wdata.bits.op.asUInt === 0.U) && !illegal_priv) // write
    {
      val pv = Cat(fromCSR.addr.bits.priv.asUInt, fromCSR.addr.bits.virt)
      when(pv === Cat(PrivType.M.asUInt, false.B)) {
        intFilesSelOH_w := UIntToOH(0.U)
      }
        .elsewhen(pv === Cat(PrivType.S.asUInt, false.B)) {
          intFilesSelOH_w := UIntToOH(1.U)
        }
        .elsewhen(pv === Cat(PrivType.S.asUInt, true.B)) {
          intFilesSelOH_w := UIntToOH(1.U((fromCSR.vgein.getWidth + 1).W)
            + fromCSR.vgein.pad(params.vgeinWidth + 1))
        }
    }
  }
      ...
      val maps = (0 until intFilesNum).map { j =>
        ...
        j * pow2(params.intFileMemWidth).toInt -> Seq(RegField(
          ...
          RegWriteFn { (valid, data) =>
            when(valid) {
              seteipnum.bits := data(params.imsicIntSrcWidth - 1, 0); seteipnum.valid := true.B
            }; true.B
          }
        ))
      }
